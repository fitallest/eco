...
import { X, Calculator, FileText, Gavel, Phone, Grid } from 'lucide-react';
...
type ToolType = 'CALCULATOR' | 'DOC_ANALYZER' | 'MOCK_TRIAL' | 'VOICE_LAWYER' | 'MENU';
      ...
      <div className="flex-1 flex flex-col w-full h-full overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900 shadow-md z-10">
          <div className="flex items-center gap-4">
             {activeTool !== 'MENU' && (
               <button onClick={() => setActiveTool('MENU')} className="text-slate-400 hover:text-white text-xs font-bold uppercase flex items-center gap-2 px-3 py-1.5 bg-slate-800 rounded-lg transition-colors">
                 <Grid size={14}/> Menu
               </button>
             )}
             <h2 className="text-xl font-bold text-white flex items-center gap-2 font-mono">
               {activeTool === 'MENU' ? 'HỆ SINH THÁI TIỆN ÍCH' : activeTool}
             </h2>
          </div>
        ...
        <div className="flex-1 p-0 overflow-hidden bg-[#020617] relative">
          ...
          <div className="relative z-10 h-full p-4 md:p-6 max-w-7xl mx-auto">
             {renderContent()}
          </div>
        </div>
      </div>
